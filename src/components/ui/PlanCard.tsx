...
const PlanCard: React.FC<PlanCardProps> = ({ data }) => {
  ...
  return (
    <motion.div whileHover={{ y: -10 }} initial={{ scale: isFeatured ? 1.05 : 1 }} className="h-100">
      <Card
        className="modern-card h-100 text-white shadow-lg position-relative overflow-hidden"
        style={{
      ...
      >
        ...
        <Card.Body className="p-4 p-lg-5 d-flex flex-column text-center">
          <h3 className="h4 fw-bold text-uppercase mb-0" style={{ letterSpacing: '2px' }}>
            {name}
          </h3>
          ...
          <ul className="list-unstyled text-start mx-auto mb-5 flex-grow-1" style={{ maxWidth: '280px' }}>
            {features.map((item, index) => (
              <li key={index} className="mb-3 d-flex align-items-start gap-2">
                ...
                <span className="small" style={{ color: '#e9ecef' }}>
                  {item}
                </span>
              </li>
            ))}
          ...
          <motion.div whileTap={{ scale: 0.95 }}>
            <Button
              ...
              style={{
                ...
                boxShadow: isFeatured ? `0 0 15px ${accentColor}40` : 'none'
              }}
            >
